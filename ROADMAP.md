...
| Deploy gate 2-axes (SOIC + Osiris) | ✅ **résolu (P9 D2) 2026-05-18** | Pattern dual-axis : séparation mesure (SOIC D1-D9 / Osiris externe) et décision (verdict joint, blocker traçable). `nexos/deploy_decision.py` + 7 placeholders dans rapport Ph5 + section doctor dédiée. Politique UNKNOWN : Osiris absent ne bloque pas. Pattern extensible. 18 tests dédiés. Commits `8ce284c` + `efb3d42` + `0bb8f2d`. |
| Preflight Osiris API alignment | ✅ **résolu (B1+) 2026-05-18** | `tools/preflight.sh:91` délègue maintenant à `tools/osiris-scan.sh` (NEW API alignée P7). Avant : tous les clients avaient `tooling/osiris.json` en erreur → verdict dual-axis dégénérait en SOIC seul. Après : score Osiris mesuré. Bug secondaire fixé : `deploy_decision.py` tolère `score` (JSON brut) ou `osiris_score` (RunStore). Commit `6b789d9`. |
| Deploy gate 4-axes (extension dual-axis) | ✅ **résolu 2026-05-18** | Ajout Lighthouse perf (≥85/100) + npm audit (HIGH+CRITICAL ≤0) comme axes 3 et 4 du verdict joint. `blocker: Blocker | None` → `blockers: list[str]`. Avant : sites avec perf=40 ou 5 CVE HIGH pouvaient passer si SOIC+Osiris OK ; après : verdict bloque proprement. 22 tests dédiés. Commit `d6a3b3f`. |
| collectif-nova Vitest + middleware restore | ✅ **résolu 2026-05-18** | `.gitignore:23` retiré → collectif-nova rejoint le tracking monorepo. Test invariants a révélé une régression cachée : middleware next-intl avait été remplacé par CSP-only par `_fix_csp_middleware` quand il n'existait pas. Restauré au pattern standard. CI matrix 7→8 clients. Commit `f468b31`. |
...
- `b4b24cc` docs(roadmap): B1+ résolu — preflight Osiris API alignment
- `d6a3b3f` feat(deploy_decision): extend to 4 axes (Lighthouse + npm audit)
- `83877a6` docs(claude.md+roadmap): extension dual-axis 4 axes
...
- **Extension dual-axis P9 D2 en 4-axes (vraie évolution du pattern)** : Lighthouse perf (≥85/100) + npm audit (HIGH+CRITICAL ≤0) ajoutés comme axes 3 et 4 du verdict joint. Refactor `blocker: Blocker | None` → `blockers: list[str]` pour exposer plusieurs FAIL en parallèle. Avant : sites avec perf=40 ou 5 CVE HIGH passaient le verdict si SOIC+Osiris OK (faux positif ACCEPT). Après : ces régressions bloquent proprement, visible dans doctor + rapport Ph5. Pattern extensible démontré.
- **collectif-nova : test invariants a servi d'auditeur** : retrait `.gitignore:23` + test Vitest a révélé une régression cachée — le middleware next-intl avait été silencieusement écrasé par `_fix_csp_middleware` (P4a) il y a plusieurs semaines (le fixer skip si middleware.ts existe, mais collectif-nova n'en avait pas au moment où il a tourné). Restauration au pattern standard. CI matrix 7→8 clients. Item de dette latent noté : `_fix_csp_middleware` devrait détecter i18n et générer un middleware combiné, ou skipper si `i18n/routing.ts` existe.
- **B1 déjà résolu en pratique** : audit du sibling `osiris/` montre playwright + chromium déjà installés dans `.venv`, blocklists/trackers.json en place. Les 8 axes Osiris produisent un score sur example.com (composite 7.9). ROADMAP item était périmé.
- **P8.6.2 pa11y multi-bg worst-case** : `_fix_pa11y_contrast` calibrait jusqu'ici contre le PREMIER bg trouvé (typiquement `surface.DEFAULT`). Refactor : pour chaque muted token, sélectionne le bg avec MIN contrast vs ce token = worst case → calibrage garantit AA partout. Test régression palette 3-bgs ajouté.
- **Axe 5 pa11y (vraie démo extensibilité)** : ajout de l'axe pa11y comme 5e dimension du verdict joint a coûté ~150 lignes code + ~120 lignes tests. Pattern réplicable confirmé. Smoke test vertex-pmo révèle immédiatement les 11 erreurs WCAG résiduelles → joint FAIL blockers=[pa11y]. Avant l'axe 5, cette dette était invisible côté deploy gate. Au prochain `nexos fix vertex-pmo`, P8.6.2 ferme la boucle (11 → 0 errors → joint ACCEPT).
- **Audit dédup P8.4** : sur les 6 noms supposés `clients dormants`, 5/6 sont des vestiges abandonnés (4 doublons UsineRH du temps où ce client était dans NEXOS_PLATFORM, transféré depuis dans repo séparé `usinerh.ca` — cf hook pre-commit "Refuse any mention of UsineRH"). Seul `la-villa-du-sous-marin` est un vrai dormant. P8.4 réduit de 6 → 1 onboard réel. Économie ~250k-1M tokens LLM. Archivage destructif laissé à décision user.
